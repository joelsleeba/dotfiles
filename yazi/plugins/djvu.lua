...
function M:seek(units)
	...
	if h and h.url == self.file.url then
		...
		ya.manager_emit(
			"peek",
			{ tostring(math.max(0, cx.active.preview.skip + step)), only_if = tostring(self.file.url) }
		)
	end
end
...
function M:preload()
	...
	local output = Command("ddjvu")
		:args({ "-format=tiff", "-quality=75", "-page=" .. tostring(self.skip + 1), tostring(self.file.url) })
